// 토큰 빌드(memoir) — src/*.json(DTCG) 을 합쳐 tokens.json 과 dist/* 를 만든다.
// winterholic-base/tokens/build.mjs 와 같은 구조. Tailwind 프리셋에 기존 memoir-frontend 클래스명(primary·secondary·surface-low·on-surface…)을 별칭으로 넣고, legacy-aliases.css 가 --background/--foreground 를 잇는다.
...
function checkContrast(map, mode) {
  ...
  const pairs = [
    ...
    ['color.text.on-pink', 'color.chip.bg-active', 4.5],
    ['color.text.on-pink', 'color.surface.blush', 4.5],
    ['color.chip.text', 'color.chip.bg', 4.5],
    ['color.chip.tag-text', 'color.chip.tag-bg', 4.5],
    ['color.chip.all-text', 'color.chip.all-bg', 4.5],
    ['color.action.interactive.text', 'color.action.interactive.bg', 2.9],
    ['color.text.primary', 'color.surface.zone', 4.5],
    ['color.text.secondary', 'color.surface.zone', 4.5],
    ['color.text.tertiary', 'color.surface.zone', 4.5],
    ['color.text.tertiary', 'color.surface.cream', 4.5],
    ['color.code.text', 'color.code.bg', 4.5],
    ['color.code.comment', 'color.code.bg', 3],
    ['color.text.link', 'color.surface.zone', 4.5],
    ['color.status.secure.text', 'color.status.secure.bg', 4.5],
    ['color.status.secure.on-solid', 'color.status.secure.solid', 4.5],
...
// ---------- 4. 직렬화 ----------
const GRADIENT_ANGLE = { paper: 'vertical', 'scrim-bottom': 'vertical', 'fade-right': 'horizontal' };
...
const cssValue = (t) => {
  ...
  switch (t.$type) {
    ...
    case 'gradient': { const name = t.path.at(-1); if (name.startsWith('blob')) return `radial-gradient(circle, ${v.map((s) => `${s.color} ${Math.round(s.position * 100)}%`).join(', ')})`; const ang = GRADIENT_ANGLE[name] ?? 'default'; return `linear-gradient(var(--${PREFIX}-gradient-angle-${ang}), ${v.map((s) => `${s.color} ${Math.round(s.position * 100)}%`).join(', ')})`; }
...
const typoClass = ([k, t]) => {
  ...
  const extra = name === 'label' ? '\n  text-transform: uppercase;' : name === 'pin' ? '\n  font-variant-numeric: tabular-nums;' : '';
  return `.${PREFIX}-${name} {\n  font-family: ${fam};\n  font-size: ${v.fontSize};\n  font-weight: ${v.fontWeight};\n  line-height: ${v.lineHeight};\n  letter-spacing: ${v.letterSpacing};${extra}\n}`;
};
const typographyCss = `/* memoir typography · 자동 생성 */
/* Plus Jakarta Sans + Pretendard 로드는 소비 앱이 한다(docs/02-typography.md). Next.js 는 --font-jakarta 를 sans 첫 항목에 넣는다. */
...
/* 글래스 유틸 (04 §4) */
.${PREFIX}-glass-floating { background: var(--${PREFIX}-glass-bg-floating); backdrop-filter: blur(var(--${PREFIX}-glass-blur-strong)); -webkit-backdrop-filter: blur(var(--${PREFIX}-glass-blur-strong)); }
.${PREFIX}-glass-modal { background: var(--${PREFIX}-glass-bg-modal); backdrop-filter: blur(var(--${PREFIX}-glass-blur)); -webkit-backdrop-filter: blur(var(--${PREFIX}-glass-blur)); }
/* 로고 워드마크: m 만 rose */
.${PREFIX}-logo .${PREFIX}-logo-mark { color: var(--${PREFIX}-color-text-brand); }
/* 반응형: 모바일에서 한 단계 내린다 */
@media (max-width: 767px) {
  .${PREFIX}-display-lg { font-size: var(--${PREFIX}-font-size-4xl); }
  .${PREFIX}-display-md { font-size: var(--${PREFIX}-font-size-4xl); }
  .${PREFIX}-display-sm { font-size: var(--${PREFIX}-font-size-3xl); }
  .${PREFIX}-headline   { font-size: var(--${PREFIX}-font-size-2xl); }
}
`;
...
// 기존 memoir-frontend tailwind.config.ts 의 색 이름을 그대로 살린다(text-primary·bg-surface-low 등이 계속 동작).
Object.assign(twColors, {
  primary: `var(${varName(['brand', 'pink'])})`,
  'primary-dark': `var(${varName(['brand', 'rose'])})`,
  secondary: `var(${varName(['brand', 'periwinkle'])})`,
  'secondary-dark': `var(${varName(['brand', 'indigo'])})`,
  'on-surface': `var(${varName(['color', 'text', 'primary'])})`,
  surface: `var(${varName(['color', 'surface', 'default'])})`,
  'surface-low': `var(${varName(['color', 'surface', 'canvas'])})`,
  'sub-pink': `var(${varName(['color', 'surface', 'blush'])})`,
  'sub-cream': `var(${varName(['color', 'surface', 'cream'])})`,
});
const preset = {
  theme: {
    extend: {
      ...
      boxShadow: { ...twScale('shadow'), soft: `var(${varName(['shadow', 'soft'])})` },
...
// 4h. legacy-aliases.css — 기존 globals.css 변수 + 자주 쓰던 hex 를 토큰으로
writeFileSync(join(DIST, 'legacy-aliases.css'), `/* memoir legacy aliases · 자동 생성
   기존 app/globals.css 의 --background/--foreground 와 .text-display/.text-headline/.text-body/.text-label 을 새 토큰에 잇는다.
   tokens.css·typography.css 다음에 로드한다. 이관이 끝나면 지운다. */
:root {
  --background: var(--${PREFIX}-color-surface-canvas);
  --foreground: var(--${PREFIX}-color-text-primary);
}
.text-display  { font: var(--${PREFIX}-font-weight-bold) var(--${PREFIX}-font-size-6xl)/var(--${PREFIX}-font-line-height-tight) var(--${PREFIX}-font-family-display); letter-spacing: var(--${PREFIX}-font-letter-spacing-tight); }
.text-headline { font: var(--${PREFIX}-font-weight-semibold) var(--${PREFIX}-font-size-3xl)/var(--${PREFIX}-font-line-height-snug) var(--${PREFIX}-font-family-display); letter-spacing: var(--${PREFIX}-font-letter-spacing-tight); }
.text-body     { font: var(--${PREFIX}-font-weight-regular) var(--${PREFIX}-font-size-md)/var(--${PREFIX}-font-line-height-relaxed) var(--${PREFIX}-font-family-sans); }
.text-label    { font: var(--${PREFIX}-font-weight-semibold) var(--${PREFIX}-font-size-xs)/var(--${PREFIX}-font-line-height-normal) var(--${PREFIX}-font-family-sans); letter-spacing: var(--${PREFIX}-font-letter-spacing-wide); text-transform: uppercase; }
`);
