...
- 기하 스팟 일러스트는 색 3개 이내: `accent.verdigris` `accent.periwinkle` `teal.100`, 선 `teal.700` 1.5px.
...
- 정본은 `assets/brand/mascot.png`, 웹은 `mascot.webp`, 정사각 소개 이미지는 `mascot-avatar-512.png`다.
- 기하 스팟 일러스트와 섞지 않고 한 화면에 둘 중 하나만 쓴다.
- 빈 상태·온보딩·404에서 160~360px로 사용한다. 64px 미만은 캐릭터 대신 로고 심볼을 쓴다.
- 검은 문서 몸체·종이 얼굴·세 줄·verdigris 북마크·periwinkle 근거 불빛의 색과 위치를 임의로 바꾸지 않는다.
